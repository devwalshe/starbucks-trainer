...
let local = 'https://starbucks-trainer.vercel.app/api/coredrinks'
...
async function allcustom(){
    try{
        const response = await fetch('https://starbucks-trainer.vercel.app/api/allnames')
        const data = await response.json()
        console.log(data)
    }
    catch{
...
dynamicURL()
if(production === 'dev'){
    localStorage.setItem('LastClicked',["https://starbucks-trainer.vercel.app/api/customizations",local,'https://starbucks-trainer.vercel.app/api/customers','https://localhost:8000/order'])
    removeAllChildNodes(document.querySelector('.items'))
    removeAllChildNodes(document.querySelector('.drinkType'))
    apiRequestForCustomizations("https://starbucks-trainer.vercel.app/api/customizations")
    apiRequest(local)
    apiRequestCustomer('https://starbucks-trainer.vercel.app/api/customers')
    postUrl ='https://starbucks-trainer.vercel.app/order'
}else
if(production=== 'live'){
